...
- (void)viewDidLoad {
    ...
    [texturedBackgroundView setBackgroundColor:[UIColor colorWithRed:246.0f/255.0f green:246.0f/255.0f blue:246.0f/255.0f alpha:1.0f]];
    ...
    PFFile *imageFile = [self.user objectForKey:kSPUserProfilePicMediumKey];
    if (imageFile) {
    ...
    [queryPhotoCount whereKey:kSPPhotoUserKey equalTo:self.user];
    ...
    */
    ...
    PFQuery *queryFollowerCount = [PFQuery queryWithClassName:kSPActivityClassKey];
    [queryFollowerCount whereKey:kSPActivityTypeKey equalTo:kSPActivityTypeFollow];
    [queryFollowerCount whereKey:kSPActivityToUserKey equalTo:self.user];
    ...
    PFQuery *queryFollowingCount = [PFQuery queryWithClassName:kSPActivityClassKey];
    [queryFollowingCount whereKey:kSPActivityTypeKey equalTo:kSPActivityTypeFollow];
    [queryFollowingCount whereKey:kSPActivityFromUserKey equalTo:self.user];
    ...
    if (![[self.user objectId] isEqualToString:[[PFUser currentUser] objectId]]) {
        ...
        PFQuery *queryIsFollowing = [PFQuery queryWithClassName:kSPActivityClassKey];
        [queryIsFollowing whereKey:kSPActivityTypeKey equalTo:kSPActivityTypeFollow];
        [queryIsFollowing whereKey:kSPActivityToUserKey equalTo:self.user];
        [queryIsFollowing whereKey:kSPActivityFromUserKey equalTo:[PFUser currentUser]];
...
- (PFQuery *)queryForTable {
    ...
    [query whereKey:kSPPhotoUserKey equalTo:self.user];
...
- (void)followButtonAction:(id)sender {
    ...
    [SPUtility followUserEventually:self.user block:^(BOOL succeeded, NSError *error) {
        if (error) {
            [self configureFollowButton];
        }
    }];
...
- (void)configureFollowButton {
    ...
    [[SPCache sharedCache] setFollowStatus:NO user:self.user];
}
...
- (void)configureUnfollowButton {
    ...
    [[SPCache sharedCache] setFollowStatus:YES user:self.user];
}
