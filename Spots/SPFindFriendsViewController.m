...
- (PFQuery *)queryForTable {
    ...
    /*
     ...
     NSMutableArray *parseEmployees = [[NSMutableArray alloc] initWithArray:kSPParseEmployeeAccounts];
     [parseEmployees removeObject:[[PFUser currentUser] objectForKey:kSPUserFacebookIDKey]];
     PFQuery *parseEmployeeQuery = [PFUser query];
     [parseEmployeeQuery whereKey:kSPUserFacebookIDKey containedIn:parseEmployees];
...
- (void)followAllFriendsButtonAction:(id)sender {
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        ...
        for (int r = 0; r < self.objects.count; r++) {
            ...
            SPFindFriendsCell *cell = (SPFindFriendsCell *)[self tableView:self.tableView cellForRowAtIndexPath:indexPath object:user];
        ...
        [SPUtility followUsersEventually:self.objects block:^(BOOL succeeded, NSError *error) {
...
- (void)unfollowAllFriendsButtonAction:(id)sender {
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        ...
        [SPUtility unfollowUsersEventually:self.objects];
